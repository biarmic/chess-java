		...
		for(int i = 0; i < 2; i++) {
			for(int j = 0; j < 8; j++) {//Placing pawns
				if(i==0) {
					...
					panel.add(piece[1][j],JLayeredPane.PALETTE_LAYER);
				}else {
					piece[0][j] = new Pawn(true,6,j);
					panel.add(piece[0][j],JLayeredPane.PALETTE_LAYER);
				}
			}
			for(int j = 0; j < 2; j++) {//Placing rooks
				if(i==0) {
					piece[1][j+8] = new Rook(false,0,j*7);
					panel.add(piece[1][j+8],JLayeredPane.PALETTE_LAYER);
				}else {
					piece[0][j+8] = new Rook(true,7,j*7);
					panel.add(piece[0][j+8],JLayeredPane.PALETTE_LAYER);
				}
			}
			for(int j = 0; j < 2; j++) {//Placing knights
				if(i==0) {
					piece[1][j+10] = new Knight(false,0,1+j*5);
					panel.add(piece[1][j+10],JLayeredPane.PALETTE_LAYER);
				}else {
					piece[0][j+10] = new Knight(true,7,1+j*5);
					panel.add(piece[0][j+10],JLayeredPane.PALETTE_LAYER);
				}
			}
			for(int j = 0; j < 2; j++) {//Placing bishops
				if(i==0) {
					piece[1][j+12] = new Bishop(false,0,2+j*3);
					panel.add(piece[1][j+12],JLayeredPane.PALETTE_LAYER);
				}else {
					piece[0][j+12] = new Bishop(true,7,2+j*3);
					panel.add(piece[0][j+12],JLayeredPane.PALETTE_LAYER);
				}
			}
			if(i==0) {//Placing queens
				piece[1][14] = new Queen(false,0,3);
				panel.add(piece[1][14],JLayeredPane.PALETTE_LAYER);
			}else {
				piece[0][14] = new Queen(true,7,3);
				panel.add(piece[0][14],JLayeredPane.PALETTE_LAYER);
			}
			if(i==0) {//Placing kings
				piece[1][15] = new King(false,0,4);
				panel.add(piece[1][15],JLayeredPane.PALETTE_LAYER);
			}else {
				piece[0][15] = new King(true,7,4);
				panel.add(piece[0][15],JLayeredPane.PALETTE_LAYER);
			}
		}
