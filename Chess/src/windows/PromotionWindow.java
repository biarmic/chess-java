...
public class PromotionWindow extends Window {
	public PromotionWindow(Pawn pawn, int prevRow, int prevColumn, Board board, Piece eating) {
		super(615, 250);
		JLabel label = new JLabel("<html><div align='center'>Choose a piece to promote your pawn</div></html>",
				JLabel.CENTER);
		...
		try {
			for (int i = 0; i < 4; i++) {
				pieces[i] = new JLabel((new ImageIcon(ImageIO
						.read(getClass().getResource("/images/pieces/" + PieceType.values()[5 - i] + "-black.png")))));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
				...
				public void mouseClicked(MouseEvent event) {
					...
					if (eating == null) {
						GameManager.writeRecord(PieceListener.getIndexLine(piece) + "." + false + "." + prevRow + "."
								+ prevColumn + "." + pawn.getRow() + "." + pawn.getColumn() + ".promote."
								+ PieceType.values()[5 - a] + "\n");
					} else {
						GameManager.writeRecord(PieceListener.getIndexLine(piece) + "." + false + "." + prevRow + "."
								+ prevColumn + "." + pawn.getRow() + "." + pawn.getColumn() + ".promote."
								+ PieceType.values()[5 - a] + "."
