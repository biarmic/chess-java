...
public class Board extends JLayeredPane {
	...
	public static void updateColors() {
		...
		for(Component comp : board.getComponentsInLayer(0)) {
			...
			if(label.getText().length()==0)
				label.setBackground(((label.getX()-50)/100+(label.getY()-50)/100)%2==0 ? ColorWindow.THEME_COLOR : ColorWindow.OPPOSITE_COLOR);
			else
		...
		for(int i = 0; i < 8; i++) {
			...
			if(piece instanceof Pawn && piece.getRow()==(colorIndex==1 ? 0 : 7)) {
				...
				if(type==PieceType.queen)
					pieces[colorIndex-1][i] = new Queen(PieceColor.values()[colorIndex],piece.getRow(),piece.getColumn());
				else if(type==PieceType.knight)
					pieces[colorIndex-1][i] = new Knight(PieceColor.values()[colorIndex],piece.getRow(),piece.getColumn());
				else if(type==PieceType.rook)
					pieces[colorIndex-1][i] = new Rook(PieceColor.values()[colorIndex],piece.getRow(),piece.getColumn());
				else
					pieces[colorIndex-1][i] = new Bishop(PieceColor.values()[colorIndex],piece.getRow(),piece.getColumn());
	...
	public Piece movePiece(Piece piece, Position destination) {
		...
		if(tiles[destination.getRow()][destination.getColumn()].placePiece(piece.getPieceType(),piece.getPieceColor())!=PieceType.empty) {
			Piece eaten = findPiece(destination.getRow(),destination.getColumn(),GameManager.turnColor.opposite().index());
			remove(eaten);
			eaten.setEaten(true);
			return eaten;
		}
		...
		for(int rotation = 0; rotation < 2; rotation++) {
			for(int direction = -1; direction < 2; direction+=2) {
				for(int i = 1; i <= maxRange; i++) {
					if(isInBoard(row+i*direction*(rotation==0 ? 0 : 1),column+i*direction*(rotation==0 ? 1 : 0))) {
						Tile current = tiles[row+i*direction*(rotation==0 ? 0 : 1)][column+i*direction*(rotation==0 ? 1 : 0)];
						if(current.getPieceType()==PieceType.empty || current.getColor()!=color)
							positions.add(new Position(row+i*direction*(rotation==0 ? 0 : 1),column+i*direction*(rotation==0 ? 1 : 0)));
			...
			for(int direction = -1; direction < 2; direction+=2) {
				for(int i = 0; i < 2; i++) {
					if(isInBoard(row+(i==0 ? 1 : 2)*direction,column+(i==0 ? 2 : 1)*rotation)) {
						...
						if(current.getPieceType()==PieceType.empty || current.getColor()!=color)
							positions.add(new Position(row+(i==0 ? 1 : 2)*direction,column+(i==0 ? 2 : 1)*rotation));
					}
				}
			}
		}
	...
	private ArrayList<Position> getCastlingRange(int row, int column) {
		...
		if(!findPiece(row,column,color.index()).hasMoved()) {
			if(findPiece(row,column-4,color.index())!=null && !findPiece(row,column-4,color.index()).hasMoved()) {
			...
			if(findPiece(row,column+3,color.index())!=null && !findPiece(row,column+3,color.index()).hasMoved()) {
				for(int i = 1; i <= 2; i++) {
					if(tiles[row][column+i].getPieceType()!=PieceType.empty)
						return positions;
				}
				...
				if(tokenizer.nextToken().equals("pawn")) {
					...
					if(Boolean.parseBoolean(tokenizer.nextToken())) {
						...
						if(Math.abs(prevRow-curRow)==2 && Math.abs(column-curColumn)==1)
							positions.add(new Position(row+(GameManager.turnColor==PieceColor.white ? -1 : 1),curColumn));
					}
				}
			}
		}
	...
	private ArrayList<Position> removeThreatenedPositions(ArrayList<Position> range, int currentRow, int currentColumn) {
	...
	public boolean isKingThreatened(PieceType movingType, int currentRow, int currentColumn, int destinationRow, int destinationColumn) {
		...
		tiles[destinationRow][destinationColumn].placePiece(previousType,previousType==PieceType.empty ? PieceColor.empty : GameManager.turnColor.opposite());
	...
	public boolean isKingThreatened() {
		...
		for(Position position : straight) {
			...
			if(tile.getPieceType()==PieceType.rook || tile.getPieceType()==PieceType.queen) {
				isThreatened = true;
				break;
			}else if(tile.getPieceType()==PieceType.king && Math.abs(kingRow-position.getRow())<=1 && Math.abs(kingColumn-position.getColumn())<=1) {
				isThreatened = true;
				break;
			}
		}
		if(!isThreatened) {
			...
			if(!isThreatened) {
				...
				if(!isThreatened) {
					for(int rotation = -1; rotation < 2; rotation+=2) {
						if(isInBoard(kingRow+(GameManager.turnColor==PieceColor.white ? -1 : 1),kingColumn+rotation)) {
							Tile tile = tiles[kingRow+(GameManager.turnColor==PieceColor.white ? -1 : 1)][kingColumn+rotation];
