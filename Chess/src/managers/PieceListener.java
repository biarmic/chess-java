...
public class PieceListener extends MouseAdapter {
	...
	public void mouseDragged(MouseEvent event) {
		if(canMove) {
			source.setLocation((int)event.getPoint().getX()+source.getX()-50,(int)event.getPoint().getY()+source.getY()-50);
		}
	}
	@Override
	...
	public void movePiece(Position destination) {
		...
		if(source instanceof King && Math.abs(difference)==2) {
			((PieceListener)board.findPiece(prevRow,difference<0 ? 7 : 0,source.getPieceColor().index()).getMouseListeners()[0]).movePiece(new Position(prevRow,difference<0 ? 5 : 3));
			...
			}else if(source instanceof Pawn && Math.abs(prevColumn-source.getColumn())==1 && eating==null && GameManager.isTwoStepAdvance()) {
				Piece prevPawn = board.getPieces()[GameManager.turnColor.opposite().index()-1][Integer.parseInt(GameManager.readLastRecordLine().substring(5,6))];
				prevPawn.setEaten(true);
				board.findTile(prevPawn.getRow(),prevPawn.getColumn()).removePiece();
				board.remove(prevPawn);
				GameManager.writeRecord(getIndexLine(source)+".false."+prevRow+"."+prevColumn+"."+source.getRow()+"."+source.getColumn()+".enpassant."+board.getPieceIndex(prevPawn,prevPawn.getPieceColor().index())+"\n");
			}else {
				if(eating==null) {
					GameManager.writeRecord(getIndexLine(source)+"."+isFirstMove+"."+prevRow+"."+prevColumn+"."+source.getRow()+"."+source.getColumn()+"\n");
				}else {
					GameManager.writeRecord(getIndexLine(source)+"."+isFirstMove+"."+prevRow+"."+prevColumn+"."+source.getRow()+"."+source.getColumn()+".eat."+board.getPieceIndex(eating,eating.getPieceColor().index())+"\n");
				}
			}
			if(eating==null)
