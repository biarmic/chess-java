...
public abstract class Piece extends JLabel {
	...
	public static void updateIcons() {
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 6; j++) {
				try {
					BufferedImage img = ImageIO.read(Piece.class.getResource(
							"/images/pieces/" + PieceType.values()[j + 1] + "-" + PieceColor.values()[i + 1] + ".png"));
					for (int w = 0; w < img.getWidth(); w++) {
						for (int h = 0; h < img.getHeight(); h++) {
							Color current = new Color(img.getRGB(w, h), true);
							if (current.getAlpha() > 0) {
								int rgb = current.getRed() == 255 ? ColorWindow.THEME_COLOR.getRGB()
										: ColorWindow.OPPOSITE_COLOR.getRGB();
								img.setRGB(w, h, rgb);
							}
						}
					}
